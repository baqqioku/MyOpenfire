...
public class ConversationManager implements Startable, ComponentEventListener {
	...
	private static final String INSERT_MESSAGE = "INSERT INTO ofMessageArchive(messageID, conversationID, fromJID, fromJIDResource, toJID, toJIDResource, sentDate, body,receipt_id) VALUES (?,?,?,?,?,?,?,?,?)";
	...
	/**
	 * Flag that indicates if messages of group chats (in MUC rooms) should be
	 * archived.
	 */
	private boolean roomArchivingEnabled;
	/**
	 * List of room names to archive. When list is empty then all rooms are
	 * archived (if roomArchivingEnabled is enabled).
	 */
	...
	/**
	 * Queue of participants that joined or left a conversation. This queue is
	 * processed by the ArchivingTask.
	 */
	...
	/**
	 * Returns true if metadata archiving is enabled. Conversation meta-data
	 * includes the participants, start date, last activity, and the count of
	 * messages sent. When archiving is enabled, all meta-data is written to the
	 * database.
	 *
	...
	/**
	 * Sets whether metadata archiving is enabled. Conversation meta-data
	 * includes the participants, start date, last activity, and the count of
	 * messages sent. When archiving is enabled, all meta-data is written to the
	 * database.
	 *
	...
	/**
	 * Returns true if one-to-one chats or group chats messages are being
	 * archived.
	 *
	 * @return true if one-to-one chats or group chats messages are being
	 *         archived.
	 */
	...
	/**
	 * Returns true if message archiving is enabled for one-to-one chats. When
	 * enabled, all messages in one-to-one conversations are stored in the
	 * database. Note: it's not possible for meta-data archiving to be disabled
	 * when message archiving is enabled; enabling message archiving
	...
	/**
	 * Sets whether message archiving is enabled. When enabled, all messages in
	 * conversations are stored in the database. Note: it's not possible for
	 * meta-data archiving to be disabled when message archiving is enabled;
	 * enabling message archiving automatically enables meta-data archiving.
	 *
	 * @param enabled
	...
	/**
	 * Returns true if message archiving is enabled for group chats. When
	 * enabled, all messages in group conversations are stored in the database
	 * unless a list of rooms was specified in {@link #getRoomsArchived()} .
	 * Note: it's not possible for meta-data archiving to be disabled when room
	 * archiving is enabled; enabling room archiving automatically enables
	 * meta-data archiving.
	 *
	...
	/**
	 * Sets whether message archiving is enabled for group chats. When enabled,
	 * all messages in group conversations are stored in the database unless a
	 * list of rooms was specified in {@link #getRoomsArchived()} . Note: it's
	 * not possible for meta-data archiving to be disabled when room archiving
	 * is enabled; enabling room archiving automatically enables meta-data
	 * archiving.
	 *
	 * @param enabled
	...
	/**
	 * Returns list of room names whose messages will be archived. When room
	 * archiving is enabled and this list is empty then messages of all local
	 * rooms will be archived. However, when name of rooms are defined in this
	 * list then only messages of those rooms will be archived.
	 *
	...
	/**
	 * Sets list of room names whose messages will be archived. When room
	 * archiving is enabled and this list is empty then messages of all local
	 * rooms will be archived. However, when name of rooms are defined in this
	 * list then only messages of those rooms will be archived.
	 *
	...
	/**
	 * Returns the number of minutes a conversation can be idle before it's
	 * ended.
	 *
	 ...
	 *
	 * @param idleTime
	 *            the max number of minutes a conversation can be idle before
	 *            it's ended.
	...
	/**
	 * Returns the maximum number of minutes a conversation can last before it's
	 * ended. Any additional messages between the participants in the chat will
	...
	/**
	 * Sets the maximum number of minutes a conversation can last before it's
	 * ended. Any additional messages between the participants in the chat will
	 * be associated with a new conversation.
	 *
	 * @param maxTime
	...
	public Conversation getConversation(long conversationID) throws NotFoundException {
		if (ClusterManager.isSeniorClusterMember()) {
			...
			// Otherwise, it might be an archived conversation, so attempt to
			// load it.
	...
	/**
	 * Returns the total number of messages that have been archived to the
	 * database.
	 *
	 ...
	 * mapped to a conversation and then queued for storage if archiving is
	 * turned on.
	 *
	 * @param sender
	 ...
	 */
	void processMessage(JID sender, JID receiver, String body, String stanza, Date date, String receipt_id) {
		...
		synchronized (conversationKey.intern()) {
			...
			if (conversation == null) {
				...
				// Check to see if this is an external conversation; i.e. one of
				// the participants
				// is on a different server. We can use XOR since we know that
				// both JID's can't
				// be external.
			...
			// Check to see if the current conversation exceeds either the max
			// idle time
			...
			if (messageArchivingEnabled) {
				if (body != null) {
					/*
					 * OF-677 - Workaround to prevent null messages being
					 * archived
					 */
					messageQueue.add(new ArchivedMessage(conversation.getConversationID(), sender, receiver, date, body, false, receipt_id));
				}
			}
	...
	/**
	 * Processes an incoming message sent to a room. The message will mapped to
	 * a conversation and then queued for storage if archiving is turned on.
	 *
	 * @param roomJID
	 *            the JID of the room where the group conversation is taking
	 *            place.
	 * @param sender
	 ...
	 *            date when the message was sent.
	 */
	void processRoomMessage(JID roomJID, JID sender, String nickname, String body, Date date, String receipt_id) {
		...
		synchronized (conversationKey.intern()) {
			...
			if (roomArchivingEnabled && (roomsArchived.isEmpty() || roomsArchived.contains(roomJID.getNode()))) {
				...
				if (body != null) {
					 ...
					 * archived
					 */
					messageQueue.add(new ArchivedMessage(conversation.getConversationID(), sender, jid, date, body, false, receipt_id));
				}
			}
			// Notify listeners of the conversation update.
			for (ConversationListener listener : conversationListeners) {
	...
	/**
	 * Notification message indicating that a user joined a groupchat
	 * conversation. If no groupchat conversation was taking place in the
	 * specified room then ignore this event.
	 * <p>
	 * <p/>
	 * Eventually, when a new conversation will start in the room and if this
	 * user is still in the room then the new conversation will detect this user
	 * and mark like if the user joined the converstion from the beginning.
	 *
	 * @param room
	...
	/**
	 * Notification message indicating that a user left a groupchat
	 * conversation. If no groupchat conversation was taking place in the
	 * specified room then ignore this event.
	 *
	 * @param room
	...
	/**
	 * Returns the group conversation taking place in the specified room or
	 * <tt>null</tt> if none.
	 *
	 * @param room
	 *            JID of the room.
	 * @return the group conversation taking place in the specified room or null
	 *         if none.
	 */
	 ...
	 * conversation manager. Only messages between two users, group chats, or
	...
	boolean isConversation(Message message) {
		if (Message.Type.normal == message.getType() || Message.Type.chat == message.getType()) {
			// TODO: how should conversations with components on other servers
			// be handled?
	...
	/**
	 * Returns a unique key for a coversation between two JID's. The order of
			...
			if (!messageQueue.isEmpty() || !conversationQueue.isEmpty() || !participantQueue.isEmpty()) {
				...
				try {
					...
					while ((message = messageQueue.poll()) != null) {
						...
						pstmt.setString(9, message.getReceipt_id());
	...
	/**
	 * A PropertyEventListener that tracks updates to Jive properties that are
	 * related to conversation tracking and archiving.
	 */
