...
/**
 * Binds a resource to the stream so that the client's address becomes a full
 * JID. Once a resource has been binded to the session the entity (i.e. client)
 * is considered a "connected resource".
 * <p>
 * Clients may specify a desired resource but if none was specified then the
 * server will create a random resource for the session. The new resource should
 * be in accordance with ResourcePrep. The server will also verify if there are
 * previous sessions from the same user that are already using the resource
 * specified by the user. Depending on the server configuration the old session
...
public class IQBindHandler extends IQHandler {
	...
	public IQ handleIQ(IQ packet) throws UnauthorizedException {
		...
		/*
		 * if (authToken.isAnonymous()) { // User used ANONYMOUS SASL so
		...
		if (authToken.isAnonymous()) {
			if (StringUtils.isNotBlank(resource))
				session.setAnonymousAuth(resource);
			else
				session.setAnonymousAuth();
		} else {
