...
class LocalSessionManager {
    ...
    private class ServerCleanupTask extends TimerTask {
		...
		public void run() {
            ...
            //Log.info("session deadline:{}",deadline);
            for (LocalIncomingServerSession session : incomingServerSessions.values()) {
                try {
                	//Log.info("session ActiveDate:{}",session.getLastActiveDate().getTime());
                    if (session.getLastActiveDate().getTime() < deadline) {
                    	//Log.info("session.getLastActiveDate().getTime():{},   deadline:{}",session.getLastActiveDate().getTime(),deadline);
